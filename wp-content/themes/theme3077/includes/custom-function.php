<?php
	...
	if (!function_exists('title_shortcode')) {
		function title_shortcode( $atts, $content = null, $shortcodename = '' ) {
			...
			if ($subtitle!="") {
				$output .= '<div class="title-box_secondary">';
				$output .= $subtitle;
				$output .= '</div>';
			}

				$output .= '<h5 class="title-box_primary">';
				$output .= $title;
				$output .= '</h5>';
		...
		function shortcode_recent_posts( $atts, $content = null, $shortcodename = '' ) {
			...
			foreach($latest as $k => $post) {
					...
					//Standard
						...
						$post_index = $custom['post-index'][0];

						if($post_index != '') {
							$output .= '<div class="post-title-with-index">';
							$output .= '<div class="post-index">'.$post_index.'</div>';
						}
						...
						$output .= '<div class="post-content-holder">';
